    ...
    <div className={cn("relative block md:hidden", className)}>
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="absolute inset-x-0 bottom-full mb-2 flex flex-col justify-center items-center gap-2"
          >
            {items.map((item, idx) => (
              <motion.div
                key={item.title}
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex h-16 items-end gap-5 rounded-xl px-4 pb-3 bg-transparent md:flex",
        className,
      )}
    >
      {items.map((item) => (
